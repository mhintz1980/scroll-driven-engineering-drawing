...
> **Status as of 2026-05-09:** Rig debug pass in progress after spatial audit.
> Active context docs have been restored from `docs/plans/archived/` to `docs/plans/`.
> Station B (Buffer Tube Socket) now uses a viewport-aware camera stop so the detail circle remains centered
> in the 35-deg tilted perspective view at the 975×550 audit viewport and larger desktop sizes.
> Playwright verification checked 975×550, 1280×720, 1440×900, and 390×844; the detail circle center
> landed within about 15px of viewport center in all checked viewports.
...
### Audit Follow-Up Notes (2026-05-09)

- Fixed context drift by restoring the active plan and handoff files from `docs/plans/archived/`.
- Replaced Station B's fixed tilted stop with a viewport-aware stop:
  - `x = -6320 + 0.495*(vw - 975)`
  - `y = -740 + 0.47*(vh - 550)`
  - `scale = 1.2`
  - `rotateX = 35`
- Confirmed DOF starts at `invert(1) blur(0px)` and animates to `invert(1) blur(10px)` on Station A lift.
- Updated `ProjectZone` reset behavior so local station animations can replay after leaving and re-entering view.
