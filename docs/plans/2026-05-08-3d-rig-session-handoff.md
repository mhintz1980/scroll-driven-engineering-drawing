...
> **2026-05-09 audit update:** Station B's original fixed tilted stop was valid only for the prior calibration viewport. The active implementation now uses a viewport-aware Station B camera stop documented below and in `.continue-here.md`.
...
- GSAP pan 2 (tilt stop / Station B): `x: -6320 + 0.495*(vw - 975), y: -740 + 0.47*(vh - 550), scale: 1.2, rotateX: 35` ← viewport-aware update after 2026-05-09 audit
...
- 2026-05-09 update: local GSAP state resets on observer exit so station animations can replay on re-entry
...
- Baseline audit viewport: `975×550`
- Camera stop: `x=-6320 + 0.495*(vw - 975), y=-740 + 0.47*(vh - 550), scale=1.2, rotateX=35`
- Baseline result: Station B detail-circle center lands within ~10px of viewport center
- Multi-viewport result: checked 975×550, 1280×720, 1440×900, and 390×844; Station B center stayed within about 15px of viewport center

## 2026-05-09 Spatial Rig Audit Fixes

**Audit source:** `/home/markimus/projects/Spatial Rig Audit Report.md`

**Issues addressed:**
- Restored active docs to `docs/plans/2026-05-07-3d-perspective-floor-plane.md` and `docs/plans/2026-05-08-3d-rig-session-handoff.md`.
- Fixed Station B circle being off-screen at the 975×550 audit viewport by making the tilted stop viewport-aware.
- Made the DOF initial state explicit with `invert(1) blur(0px)` before lift.
- Made `ProjectZone` station animations replayable by resetting state when a station leaves view.

**Verification:**
- Focused lint passed for `DrawingPackagePage.tsx` and `ProjectZone.tsx`.
- `npm run build` passed.
- Playwright confirmed Station B circle centering across 975×550, 1280×720, 1440×900, and 390×844.
- Full lint/test still have unrelated pre-existing failures outside the spatial rig fix scope.
...
- Station B uses viewport-aware coordinates because the 35° perspective projection shifts the detail circle as viewport size changes
